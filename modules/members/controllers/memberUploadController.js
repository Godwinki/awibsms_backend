...
exports.generateTemplate = (req, res) => {
  try {
    const headers = [
      ...
      'Account Number', 
      'Phone Number',
      'Branch (Auto-assigned from selection)'
    ];
    ...
    const exampleRow = [
      ...
      '255712345678',
      'Will be assigned from your branch selection'
    ];
...
exports.uploadMembers = async (req, res) => {
  ...
  uploadMiddleware(req, res, async (err) => {
    ...
    if (!branchId) {
      return res.status(400).json({
        success: false,
        message: 'Branch ID is required for member upload.'
      });
    }
      ...
      try {
        for (const row of data) {
          ...
          const memberData = {
            ...
            branchId: branchId, // Add the branch ID from request
