...
// Get a single document by ID (for CMS management)
exports.getDocumentById = async (req, res) => {
  console.log(`📥 [PublicDocument] Request to get document: ${req.params.id}`);
  try {
    const document = await PublicDocument.findByPk(req.params.id, {
      include: [
        {
    ...
    console.log(`✅ [PublicDocument] Document found: ${document.title}`);
    res.json({ 
      emoji: '📄', 
    ...
    console.log('❌ [PublicDocument] Failed to fetch document:', error.message);
    res.status(500).json({ emoji: '❌', error: error.message });
  }
};
